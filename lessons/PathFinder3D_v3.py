...
def render_wall_slice(screen, x, start_pos, height, distance, wall_x):
    if USE_TEXTURE or True:  # We'll use our texture either way now
        ...
        # Scale the slice to the correct height
        if height > 0:
            wall_slice = pygame.transform.scale(wall_slice, (1, int(height)))

            # Apply distance shading
            shade = max(0, min(255, 255 - int(distance * 20)))
            wall_slice.fill((shade, shade, shade), special_flags=pygame.BLEND_MULT)

            screen.blit(wall_slice, (x, start_pos))
    else:
        # Fallback to simple colored walls
        color = max(0, min(255, 255 - int(distance * 20)))
        pygame.draw.line(screen, (color, color, color), (x, start_pos), (x, start_pos + height))
...
def main():
    ...
    while running:
        ...
        for x in range(SCREEN_WIDTH):
            ray_angle = (player.angle - player.fov/2) + (x/SCREEN_WIDTH) * player.fov
            ...
            if wall_type == 3:  # End point
                color = max(0, min(255, 255 - int(dist * 20)))
